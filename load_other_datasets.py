...
def load_LE_dataset(path="../Hypergraph GNN/NeurIPS_Code/data/", dataset="ModelNet40", train_percent=0.025):
    ...
    data.edge_index, data.edge_attr = coalesce(data.edge_index, None, total_num_node_id_he_id, total_num_node_id_he_id)
...
def load_yelp_dataset(
    path="../data/raw_data/yelp_raw_datasets/", dataset="yelp", name_dictionary_size=1000, train_percent=0.025
):
    """
...
def load_cornell_dataset(
    path="../data/raw_data/", dataset="amazon", feature_noise=0.1, feature_dim=None, train_percent=0.025
):
    """
    this will read the yelp dataset from source files, and convert it edge_list to
    [[ -V- | -E- ]
