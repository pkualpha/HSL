...
class dataset_Hypergraph(InMemoryDataset):
    def __init__(
        self,
        root="/data/0shared/caiderun/HSL-data/",
        name=None,
        p2raw="data/raw/",
        train_percent=0.01,
        feature_noise=None,
            ...
            if not osp.isfile(p2f):
                ...
                if self.name in ["cora", "citeseer", "pubmed"]:
                    tmp_data = load_citation_dataset(
                        path=os.path.join(self.p2raw, "cocitation"),
                        dataset=self.name,
                    ...
                    tmp_data = load_citation_dataset(
                        path=os.path.join(self.p2raw, "coauthorship"),
                    ...
                    tmp_data = load_yelp_dataset(
                        path=os.path.join(self.p2raw, "yelp"), dataset=self.name, train_percent=self._train_percent
                    )
...
if __name__ == "__main__":
    ...
    for f in [
        "walmart-trips-100",
    ]:  # 'house-committees', 'amazon-reviews']:
        for feature_noise in [0.1, 1]:
            dd = dataset_Hypergraph(root=p2root, name=f, feature_noise=feature_noise, p2raw=p2raw)
    ...
    for f in [
        ...
        dd = dataset_Hypergraph(root=p2root, name=f, p2raw=p2raw)
        assert dd.data.num_nodes in dd.data.edge_index[0]
        print(dd, dd.data)
